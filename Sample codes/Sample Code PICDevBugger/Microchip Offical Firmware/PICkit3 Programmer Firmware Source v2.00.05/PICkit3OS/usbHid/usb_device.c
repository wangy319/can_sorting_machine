...
// Device Descriptor: for cloning the ROM image to RAM so we can override parameters
USB_DEVICE_DESCRIPTOR device_dsc_RAM;
...
void USBDeviceInit(void)
{
    ...
    U1IE = 0x9F;                    // and resume (hub issue)
    ...
    if(USBTransactionCompleteIE)
    {
        ...
        {                       //utilization can be compromised, and the device won't be able to receive SETUP packets.
            if(USBTransactionCompleteIF)
            {
                ...
                USBCtrlEPService();
                /*
                 * USBEPService() services the rest of the end points
                 *
                 */
                 USBEPService();
...
USB_SERIAL_NUMBER_DESCRIPTOR gSerialNumberDescriptor;

void USBSetSerialNumberDescriptorInRam(void)
{
    word i;
    word size;
    byte *pRamPtr = (byte *)&gSerialNumberDescriptor;
    ROM byte *pRomPtr;
    ROM USB_SERIAL_NUMBER_DESCRIPTOR *pRomPtrToSerialNumberDescriptor;
    
    pRomPtrToSerialNumberDescriptor = (ROM USB_SERIAL_NUMBER_DESCRIPTOR *)USB_SD_Ptr[3];
    size = pRomPtrToSerialNumberDescriptor->bLength;
    pRomPtr = USB_SD_Ptr[3];
...
// sn must be USB_SERIAL_NUMBER_LENGTH_IN_WORDS words long
void USBSetSerialNumber(word *sn)
{
    byte i;
    
    for (i = 0 ; i < USB_SERIAL_NUMBER_LENGTH_IN_WORDS; i++)
        gSerialNumberDescriptor.string[i] = *sn++;
}   
    ...
    if(SetupPkt.bmRequestType == 0x80)
    {
        ...
        switch(SetupPkt.bDescriptorType)
        {
            case USB_DESCRIPTOR_DEVICE:
                    ...
                    // Programmatically steer where (in memory)
                    // the USB Device Descriptor is obtained from:
                    //
                    //      1) From RAM for PKOB (PICKit 3 On-Board) units
                    //                  or
                    //      2) From ROM for PICkit 3 units    
            ...
            case USB_DESCRIPTOR_STRING:
                // 'Serial Number' descriptor string test
                if (SetupPkt.bDscIndex == 3)
                {
                    // The serial number comes from RAM (so we can change it!)
                    inPipes[0].info.Val &= ~USB_INPIPES_ROM;
                    inPipes[0].info.Val |=  USB_INPIPES_RAM;
                    inPipes[0].pSrc.bRam = (BYTE *)&gSerialNumberDescriptor;
